...
class dataset_builder():
    ...
    def get_data(self):
        ...
        prot_ser = np.load(self.data_path['prot_ser'], allow_pickle=True)
    ...
    def transform_dataset(self,
        ...
        if is_prot_bpe:
            protein_data = self.encoding_bpe(PROTS, DATA[4],
                                            DATA[5], prot_max_len)
        else:
            protein_data = torch.IntTensor(np.array(self.data_conversion(PROTS,
                                            DATA[1], prot_max_len)))
